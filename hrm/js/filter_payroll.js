...
const TARGET_BLOCK_UID = '7vaeykvra1c';

// ─── CONFIG: AI modifies here ────────────────────────────────
const COLLECTION = 'hrm_payroll';

const GROUPS = [
  { name: 'status', items: [
    { key: 'all', label: 'All', filter: null },
    { key: 'draft', label: 'Draft', filter: { status: { $eq: 'Draft' } } },
    { key: 'calculated', label: 'Calculated', filter: { status: { $eq: 'Calculated' } } },
    { key: 'approved', label: 'Approved', filter: { status: { $eq: 'Approved' } } },
    { key: 'paid', label: 'Paid', filter: { status: { $eq: 'Paid' } } },
  ]},
];
// ─── CONFIG END ────────────────────────────────────
