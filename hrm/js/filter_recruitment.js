...
const TARGET_BLOCK_UID = '024na8lxr82';

// ─── CONFIG: AI modifies here ────────────────────────────────
const COLLECTION = 'hrm_recruitment';

const GROUPS = [
  { name: 'status', items: [
    { key: 'all', label: 'All', filter: null },
    { key: 'open', label: 'Open', filter: { status: { $eq: 'Open' } } },
    { key: 'screening', label: 'Screening', filter: { status: { $eq: 'Screening' } } },
    { key: 'interviewing', label: 'Interviewing', filter: { status: { $eq: 'Interviewing' } } },
    { key: 'offer', label: 'Offer', filter: { status: { $eq: 'Offer' } } },
