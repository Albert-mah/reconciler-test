...
const TARGET_BLOCK_UID = '35tf1aeye0t';

// ─── CONFIG: AI modifies here ────────────────────────────────
const COLLECTION = 'hrm_attendance';

const GROUPS = [
  { name: 'status', items: [
    { key: 'all', label: 'All', filter: null },
    { key: 'present', label: 'Present', filter: { status: { $eq: 'Present' } } },
    { key: 'late', label: 'Late', filter: { status: { $eq: 'Late' } } },
    { key: 'absent', label: 'Absent', filter: { status: { $eq: 'Absent' } }, danger: true },
    { key: 'leave', label: 'Leave', filter: { status: { $eq: 'Leave' } } },
  ]},
];
// ─── CONFIG END ────────────────────────────────────
