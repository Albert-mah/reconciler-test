/**
 * Material Stats Filter
 *
 * @type JSItemModel
 * @template filter-stats
 */

const TARGET_BLOCK_UID = 'coozl7m9ex7';

// ─── CONFIG: AI modifies here ────────────────────────────────
const COLLECTION = 'erp_materials';
...
const GROUPS = [
  {
    ...
    items: [
      ...
      { key: 'dev', label: 'In Development', filter: { status: { $eq: 'In Development' } } },
      { key: 'discontinued', label: 'Discontinued', filter: { status: { $eq: 'Discontinued' } } },
  ...
  },
  {
    name: 'Alert',
    items: [
      { key: 'low_stock', label: 'Low Stock', filter: { $and: [{ stock_qty: { $lt: { $col: 'min_stock' } } }] }, danger: true },
    ],
  },
];
// ─── CONFIG END ────────────────────────────────────
