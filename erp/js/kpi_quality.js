/**
 * Monthly Quality Pass Rate KPI Card
 *
 * @type JSBlockModel
 * @template kpi-card
 */
...
const COLOR = '#8b5cf6';
const BG = '#f5f3ff';
const SQL_UID = 'erp_kpi_quality';
const FMT = (v) => `${v.toFixed(1)}%`;

const buildSql = (startDate, endDate, prevStart, prevEnd) => `
SELECT
  CASE WHEN COALESCE(SUM(CASE WHEN created_at >= '${startDate}' AND created_at <= '${endDate}'
    THEN sample_qty ELSE 0 END), 0) > 0
  THEN ROUND(
    SUM(CASE WHEN created_at >= '${startDate}' AND created_at <= '${endDate}' THEN pass_qty ELSE 0 END)::numeric
    / SUM(CASE WHEN created_at >= '${startDate}' AND created_at <= '${endDate}' THEN sample_qty ELSE 0 END) * 100, 1)
  ELSE 0 END as current_value,
  CASE WHEN COALESCE(SUM(CASE WHEN created_at >= '${prevStart}' AND created_at < '${prevEnd}'
    THEN sample_qty ELSE 0 END), 0) > 0
    AND COALESCE(SUM(CASE WHEN created_at >= '${startDate}' AND created_at <= '${endDate}'
    THEN sample_qty ELSE 0 END), 0) > 0
  THEN ROUND(
    (SUM(CASE WHEN created_at >= '${startDate}' AND created_at <= '${endDate}' THEN pass_qty ELSE 0 END)::numeric
     / SUM(CASE WHEN created_at >= '${startDate}' AND created_at <= '${endDate}' THEN sample_qty ELSE 0 END)
    - SUM(CASE WHEN created_at >= '${prevStart}' AND created_at < '${prevEnd}' THEN pass_qty ELSE 0 END)::numeric
     / SUM(CASE WHEN created_at >= '${prevStart}' AND created_at < '${prevEnd}' THEN sample_qty ELSE 0 END))
    * 100, 1)
