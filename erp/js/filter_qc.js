...
const GROUPS = [
  {
    ...
    items: [
      ...
      { key: 'pass', label: 'Pass', filter: { result: { $eq: 'Pass' } } },
      { key: 'fail', label: 'Fail', filter: { result: { $eq: 'Fail' } }, danger: true },
      { key: 'conditional', label: 'Conditional Accept', filter: { result: { $eq: 'Conditional Accept' } } },
      { key: 'pending', label: 'Pending', filter: { result: { $eq: 'Pending' } } },
    ],
  },
];
// ─── CONFIG END ────────────────────────────────────
