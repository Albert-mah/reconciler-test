/**
 * Supplier Stats Filter
 *
 * @type JSItemModel
 * @template filter-stats
 */

const TARGET_BLOCK_UID = 'm6h4ncv9igc';

// ─── CONFIG: AI modifies here ────────────────────────────────
const COLLECTION = 'erp_suppliers';
...
const GROUPS = [
  {
    ...
    items: [
      ...
      { key: 'a', label: 'A-Excellent', filter: { rating: { $eq: 'A-Excellent' } } },
      { key: 'b', label: 'B-Good', filter: { rating: { $eq: 'B-Good' } } },
      { key: 'c', label: 'C-Acceptable', filter: { rating: { $eq: 'C-Acceptable' } } },
      { key: 'd', label: 'D-Needs Improvement', filter: { rating: { $eq: 'D-Needs Improvement' } }, danger: true },
    ],
  },
];
// ─── CONFIG END ────────────────────────────────────
