...
const TARGET_BLOCK_UID = 'nez66vncbzg';

// ─── CONFIG: AI modifies here ────────────────────────────────
const COLLECTION = 'erp_sales_orders';
...
const GROUPS = [
  {
    ...
    items: [
      ...
      { key: 'confirmed', label: 'Confirmed', filter: { status: { $eq: 'Confirmed' } } },
      { key: 'production', label: 'In Production', filter: { status: { $eq: 'In Production' } } },
      { key: 'ready', label: 'Ready to Ship', filter: { status: { $eq: 'Ready to Ship' } } },
      { key: 'partial', label: 'Partially Shipped', filter: { status: { $eq: 'Partially Shipped' } } },
      { key: 'shipped', label: 'Shipped', filter: { status: { $eq: 'Shipped' } } },
      { key: 'delivered', label: 'Delivered', filter: { status: { $eq: 'Delivered' } } },
      { key: 'cancelled', label: 'Cancelled', filter: { status: { $eq: 'Cancelled' } }, danger: true },
    ],
  },
];
// ─── CONFIG END ────────────────────────────────────
