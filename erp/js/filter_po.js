...
const TARGET_BLOCK_UID = 'ig1801dquea';

// ─── CONFIG: AI modifies here ────────────────────────────────
const COLLECTION = 'erp_purchase_orders';
...
const GROUPS = [
  {
    ...
    items: [
      ...
      { key: 'pending', label: 'Pending Approval', filter: { status: { $eq: 'Pending Approval' } } },
      { key: 'approved', label: 'Approved', filter: { status: { $eq: 'Approved' } } },
      { key: 'partial', label: 'Partial Receipt', filter: { status: { $eq: 'Partial Receipt' } } },
      { key: 'received', label: 'Received', filter: { status: { $eq: 'Received' } } },
