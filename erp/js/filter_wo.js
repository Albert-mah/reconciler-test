...
const TARGET_BLOCK_UID = 'ivkvlkwrwzn';

// ─── CONFIG: AI modifies here ────────────────────────────────
const COLLECTION = 'erp_work_orders';
...
const GROUPS = [
  {
    ...
    items: [
      ...
      { key: 'scheduled', label: 'Scheduled', filter: { status: { $eq: 'Scheduled' } } },
      { key: 'progress', label: 'In Progress', filter: { status: { $eq: 'In Progress' } } },
      { key: 'hold', label: 'On Hold', filter: { status: { $eq: 'On Hold' } }, danger: true },
