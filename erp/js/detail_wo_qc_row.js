 ...
 * @fields qc_type, result, pass_qty, sample_qty, batch_no, inspector
 */
const { Tag, Space, Badge, Typography } = ctx.antd;
const { Text } = Typography;
const { qc_type, result, pass_qty, sample_qty, batch_no, inspector } = ctx.record;
...
ctx.render(
  <Space size="middle" align="center" style={{ width: '100%' }}>
    ...
    <Text style={{ fontSize: 13 }}>
      <Text type="secondary">检验员:</Text> {inspector || '-'}
    </Text>
  </Space>
);
