...
const CONFIG = {
  title: '本月销售额',              // 卡片标题
  gradient: ['#1677ff', '#4096ff'], // 渐变色 [起始, 结束]
  textColor: '#fff',                // 文字颜色
  prefix: '¥',                      // 数值前缀 (¥, 件, %)
  suffix: '',                        // 数值后缀
};
...
const SQL = `
SELECT
  COALESCE(SUM(CASE WHEN order_date >= :__var1 AND order_date <= :__var2
    THEN total_amount ELSE 0 END), 0) as current_value,
  COALESCE(SUM(CASE WHEN order_date >= :__var3 AND order_date < :__var4
    THEN total_amount ELSE 0 END), 0) as previous_value
