...
// ─── CONFIG: AI 修改这里 ───────────────────────────
const COLLECTION = 'nb_erp_work_orders';
...
const GROUPS = [
  {
    ...
    items: [
      ...
      { key: 'waiting', label: '待排产', filter: { status: { $eq: '待排产' } } },
      { key: 'running', label: '生产中', filter: { status: { $eq: '生产中' } } },
      { key: 'paused', label: '暂停', filter: { status: { $eq: '暂停' } }, danger: true },
      { key: 'done', label: '已完工', filter: { status: { $eq: '已完工' } } },
    ],
  },
];
// ─── CONFIG END ────────────────────────────────────
