...
const TARGET_BLOCK_UID = '85pvyxmyxm8';

// ─── CONFIG: AI modifies here ────────────────────────────────
const COLLECTION = 'erp_customers';
...
const GROUPS = [
  {
    ...
    items: [
      ...
      { key: 'vip', label: 'VIP', filter: { level: { $eq: 'VIP' } } },
      { key: 'a', label: 'A', filter: { level: { $eq: 'A' } } },
      { key: 'b', label: 'B', filter: { level: { $eq: 'B' } } },
      { key: 'c', label: 'C', filter: { level: { $eq: 'C' } } },
    ],
  },
  {
    name: 'Status',
    items: [
      ...
      { key: 'suspended', label: 'Suspended', filter: { status: { $eq: 'Suspended' } } },
      { key: 'blocked', label: 'Blocked', filter: { status: { $eq: 'Blocked' } }, danger: true },
    ],
  },
];
// ─── CONFIG END ────────────────────────────────────
