...
const LABEL = 'Monthly Output';
const COLOR = '#10b981';
const BG = '#ecfdf5';
const SQL_UID = 'erp_kpi_production';
const FMT = (v) => v >= 1e6 ? `${(v/1e6).toFixed(1)}M` : v >= 1e3 ? `${(v/1e3).toFixed(1)}K` : `${v.toFixed(0)} pcs`;

const buildSql = (startDate, endDate, prevStart, prevEnd) => `
SELECT
  COALESCE(SUM(CASE WHEN actual_end >= '${startDate}' AND actual_end <= '${endDate}'
    THEN completed_qty ELSE 0 END), 0) as current_value,
  CASE WHEN COALESCE(SUM(CASE WHEN actual_end >= '${prevStart}' AND actual_end < '${prevEnd}'
    THEN completed_qty ELSE 0 END), 0) > 0
  THEN ROUND(
    (COALESCE(SUM(CASE WHEN actual_end >= '${startDate}' AND actual_end <= '${endDate}' THEN completed_qty ELSE 0 END), 0)
    - COALESCE(SUM(CASE WHEN actual_end >= '${prevStart}' AND actual_end < '${prevEnd}' THEN completed_qty ELSE 0 END), 0))::numeric
    / COALESCE(SUM(CASE WHEN actual_end >= '${prevStart}' AND actual_end < '${prevEnd}' THEN completed_qty ELSE 0 END), 1) * 100, 1)
