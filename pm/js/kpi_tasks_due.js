...
const LABEL = 'Tasks Due This Week';
const COLOR = '#f59e0b';
const BG = '#fffbeb';
const SQL_UID = 'pm_kpi_tasks_due';
...
const getDates = () => {
  ...
  return {
    ...
    prevStart: now.subtract(1, 'week').startOf('week').format('YYYY-MM-DD 00:00:00'),
    prevEnd: now.startOf('week').format('YYYY-MM-DD 00:00:00'),
  };
};

const buildSql = (startDate, endDate, prevStart, prevEnd) => `
SELECT
  COALESCE(COUNT(CASE WHEN due_date >= '${startDate}' AND due_date <= '${endDate}' AND status NOT IN ('Done') THEN 1 END), 0) as current_value,
  CASE WHEN COALESCE(COUNT(CASE WHEN due_date >= '${prevStart}' AND due_date < '${prevEnd}' AND status NOT IN ('Done') THEN 1 END), 0) > 0
  THEN ROUND(
    (COALESCE(COUNT(CASE WHEN due_date >= '${startDate}' AND due_date <= '${endDate}' AND status NOT IN ('Done') THEN 1 END), 0)
    - COALESCE(COUNT(CASE WHEN due_date >= '${prevStart}' AND due_date < '${prevEnd}' AND status NOT IN ('Done') THEN 1 END), 0))::numeric
    / COALESCE(COUNT(CASE WHEN due_date >= '${prevStart}' AND due_date < '${prevEnd}' AND status NOT IN ('Done') THEN 1 END), 1) * 100, 1)
...
const KpiCard = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { startDate, endDate, prevStart, prevEnd } = getDates();
