...
const COLOR = '#22c55e';
const BG = '#f0fdf4';
const SQL_UID = 'pm_kpi_budget_util';
...
SELECT
  COALESCE(SUM(CASE WHEN status = 'Active' AND "createdAt" <= '${endDate}' THEN budget ELSE 0 END), 0) as current_value,
  CASE WHEN COALESCE(SUM(CASE WHEN status = 'Active' AND "createdAt" <= '${prevEnd}' THEN budget ELSE 0 END), 0) > 0
  THEN ROUND(
    (COALESCE(SUM(CASE WHEN status = 'Active' AND "createdAt" <= '${endDate}' THEN budget ELSE 0 END), 0)
    - COALESCE(SUM(CASE WHEN status = 'Active' AND "createdAt" <= '${prevEnd}' THEN budget ELSE 0 END), 0))::numeric
    / COALESCE(SUM(CASE WHEN status = 'Active' AND "createdAt" <= '${prevEnd}' THEN budget ELSE 0 END), 1) * 100, 1)
