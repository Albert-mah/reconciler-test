...
const LABEL = 'Hours This Month';
const COLOR = '#8b5cf6';
const BG = '#f5f3ff';
const SQL_UID = 'pm_kpi_hours_logged';
...
const getDates = () => {
  ...
  return {
    ...
    prevStart: now.subtract(1, 'month').startOf('month').format('YYYY-MM-DD 00:00:00'),
    prevEnd: now.startOf('month').format('YYYY-MM-DD 00:00:00'),
  };
};

const buildSql = (startDate, endDate, prevStart, prevEnd) => `
SELECT
  COALESCE(SUM(CASE WHEN date >= '${startDate}' AND date <= '${endDate}' THEN hours ELSE 0 END), 0) as current_value,
  CASE WHEN COALESCE(SUM(CASE WHEN date >= '${prevStart}' AND date < '${prevEnd}' THEN hours ELSE 0 END), 0) > 0
  THEN ROUND(
    (COALESCE(SUM(CASE WHEN date >= '${startDate}' AND date <= '${endDate}' THEN hours ELSE 0 END), 0)
    - COALESCE(SUM(CASE WHEN date >= '${prevStart}' AND date < '${prevEnd}' THEN hours ELSE 0 END), 0))::numeric
    / COALESCE(SUM(CASE WHEN date >= '${prevStart}' AND date < '${prevEnd}' THEN hours ELSE 0 END), 1) * 100, 1)
