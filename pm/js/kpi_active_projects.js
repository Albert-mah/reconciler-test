...
const LABEL = 'Active Projects';
const COLOR = '#3b82f6';
const BG = '#eff6ff';
const SQL_UID = 'pm_kpi_active_projects';
...
SELECT
  COALESCE(COUNT(CASE WHEN status = 'Active' AND "createdAt" <= '${endDate}' THEN 1 END), 0) as current_value,
  CASE WHEN COALESCE(COUNT(CASE WHEN status = 'Active' AND "createdAt" <= '${prevEnd}' THEN 1 END), 0) > 0
  THEN ROUND(
    (COALESCE(COUNT(CASE WHEN status = 'Active' AND "createdAt" <= '${endDate}' THEN 1 END), 0)
    - COALESCE(COUNT(CASE WHEN status = 'Active' AND "createdAt" <= '${prevEnd}' THEN 1 END), 0))::numeric
    / COALESCE(COUNT(CASE WHEN status = 'Active' AND "createdAt" <= '${prevEnd}' THEN 1 END), 1) * 100, 1)
